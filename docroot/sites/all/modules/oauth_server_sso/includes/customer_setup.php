...
class MiniorangeOAuthServerCustomer {
  ...
  public function checkCustomer() {
    ...
    $url = MiniorangeOAuthServerConstants::BASE_URL . '/moas/rest/customer/check-if-exists';
    $ch = curl_init($url);
    $email = $this->email;
    ...
    if (curl_errno($ch)) {
      ...
      watchdog('oauth_server_sso', 'Error at %method of %file: %error', $error);
    }
    curl_close($ch);
  ...
  public function createCustomer() {
	...
	$proxy_host_name = variable_get('oauth_server_sso_proxy_host','');
	$port_number = variable_get('oauth_server_sso_proxy_port','');
	$username = variable_get('oauth_server_sso_proxy_username');
	$password = variable_get('oauth_server_sso_proxy_password','');
	
    $url = MiniorangeOAuthServerConstants::BASE_URL . '/moas/rest/customer/add';
    $ch = curl_init($url);

    $fields = array(
      'companyName' => $_SERVER['SERVER_NAME'],
      'areaOfInterest' => 'Drupal OAuth Server Plugin',
      'email' => $this->email,
      'phone' => $this->phone,
  ...
  public function getCustomerKeys() {
    ...
    $url = MiniorangeOAuthServerConstants::BASE_URL . '/moas/rest/customer/key';
    $ch = curl_init($url);
    $email = $this->email;
    $password = $this->password;
  ...
  public function sendOtp() {
    ...
    $url = MiniorangeOAuthServerConstants::BASE_URL . '/moas/api/auth/challenge';
    $ch = curl_init($url);
  ...
  public function validateOtp($transaction_id) {
    ...
    $url = MiniorangeOAuthServerConstants::BASE_URL . '/moas/api/auth/validate';
    $ch = curl_init($url);
